...
! function(factory) { "function" == typeof define && define.amd ? define(["jquery"], factory) : "object" == typeof exports ? module.exports = factory(require("jquery")) : factory(jQuery) }((function($) {
    function create() { return $($.map(arguments, $.proxy(document, "createElement"))) } var Checkboxpicker = function() {
            function Checkboxpicker(element, options) { this.element = element, this.$element = $(element); var data = this.$element.data(); "" === data.reverse && (data.reverse = !0), "" === data.switchAlways && (data.switchAlways = !0), "" === data.html && (data.html = !0), this.options = $.extend({}, $.fn.checkboxpicker.defaults, options, data), this.$element.closest("label").length ? console.warn(this.options.warningMessage) : (this.$group = $(document.createElement("div")), this.$buttons = create("button", "button").attr("type", "button").attr("tabindex", -1), this.$off = this.$buttons.eq(this.options.reverse ? 1 : 0), this.$on = this.$buttons.eq(this.options.reverse ? 0 : 1), this.init()) } var _proto = Checkboxpicker.prototype; return _proto.init = function() { var fn = this.options.html ? "html" : "text";
                ...
                (this.element.defaultChecked && this.$off.hasClass("active") || !this.element.defaultChecked && this.$on.hasClass("active")) && this.set(this.element.defaultChecked) }, Checkboxpicker }(),
        oldPropHooks = $.extend({}, $.propHooks); return $.extend($.propHooks, { checked: { set: function(element, value) { var data = $.data(element, "bs.checkbox");
                data && element.checked !== value && data.change(value), oldPropHooks.checked && oldPropHooks.checked.set && oldPropHooks.checked.set(element, value) } }, disabled: { set: function(element, value) { var data = $.data(element, "bs.checkbox");
                data && element.disabled !== value && data.toggleDisabled(), oldPropHooks.disabled && oldPropHooks.disabled.set && oldPropHooks.disabled.set(element, value) } } }), $.fn.checkboxpicker = function(options, elements) { return (this instanceof $ ? this : $("string" == typeof options ? options : elements)).each((function() { var data = $.data(this, "bs.checkbox");
            data || (data = new Checkboxpicker(this, options), $.data(this, "bs.checkbox", data)) })) }, $.fn.checkboxpicker.defaults = { baseGroupCls: "btn-group", baseCls: "btn", groupCls: null, cls: null, offCls: "btn-default", onCls: "btn-default", offActiveCls: "btn-success", onActiveCls: "btn-success", offLabel: "No", onLabel: "Yes", offTitle: !1, onTitle: !1, iconCls: "glyphicon", toggleKeyCodes: [13, 32], warningMessage: "Please do not use Bootstrap-checkbox element in label element." }, Checkboxpicker }));