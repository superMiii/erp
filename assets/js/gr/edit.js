...
function validasi(i) {
    var n_delivery = parseFloat($('#n_delivery'+i).val());
        ...
        success: function(data) {
            if (data['data'] != null) {
                ...
                $.each(data['data'], function(k, v) {
                    ...
                    cols += `<td><input type="text" class="form-control text-right form-control-sm" id="v_price${i}" name="v_price[]" value="${number_format(v['v_product_mill'], 2, '.', ',')}"  onkeypress="return bilanganasli(event);hitung();" onkeyup="change_ndisc(${i});hitung();" onblur=\"if(this.value==''){this.value='0';hitung();}\" onfocus=\"if(this.value=='0'){this.value='';}\">
                    ...
                    <input type="number" autocomplete="off" class="form-control text-center form-control-sm" id="n_delivery${i}" value="${x_deliver}" name="n_delivery[]" onkeyup="change_ndisc(${i});hitung();raya(${i});" onblur=\"if(this.value==''){this.value='0';hitung();}\" onfocus=\"if(this.value=='0'){this.value='';}\">
                    </td>`;

                    cols += `<td><input type="number" autocomplete="off" class="form-control text-center form-control-sm" id="bonus${i}" value="${v['x_bonus']}" name="bonus[]" onkeyup="" onblur=\"if(this.value==''){this.value='0';hitung();}\" onfocus=\"if(this.value=='0'){this.value='';}\">
                    </td>`;
                    cols += `<td><input type="text" class="formatrupiah form-control text-right form-control-sm" id="n_disc${i}" name="n_disc[]" value="${number_format(v['n_gr_discount'], 2, '.', ',')}" onkeyup="formatrupiahkeyup(this);change_ndisc(${i});hitung();" onkeydown="formatrupiahkeydown(this);change_ndisc(${i});hitung();"  onblur="if(this.value==''){this.value='0';hitung();}" onfocus="if(this.value=='0'){this.value='';}"></td>`;
                    cols += `<td><input type="text" class="formatrupiah form-control text-right form-control-sm" id="v_disc${i}" name="v_disc[]" value="${number_format(v['v_gr_discount'], 2, '.', ',')}" onkeyup="formatrupiahkeyup(this);change_vdisc(${i});hitung();" onkeydown="formatrupiahkeydown(this);change_vdisc(${i});hitung();"  onblur="if(this.value==''){this.value='0';hitung();}" onfocus="if(this.value=='0'){this.value='';}"></td>`;
                    cols += `<td><input type="text" class="form-control text-right form-control-sm" id="e_remark${i}" name="e_remark[]" value="${v['e_remark']}" ></td>`;
...
function raya(i) {
    var dell=parseFloat(formatulang($('#n_delivery'+i).val()));
    var op=parseFloat(formatulang($('#n_op'+i).val()));
    if (dell > op) {
        Swal.fire('Maaf Pesanan = ' + $('#n_op' + i).val() + ' , Jika menerima lebih dari Pesanan silahkan masukan ke BONUS atau REVISI PESANAN.');
        $('#n_delivery' + i).val($('#n_op' + i).val());
    }
}
