...
class Bbr extends CI_Controller
{
	public $i_menu = '606';
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan Data ' . $this->title . ' ID : ' . $this->input->post('i_document') . ' i_supplier : ' . $this->input->post('i_supplier'). ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
		...
		if ($this->form_validation->run() == false) {
			...
			// } else {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$this->logger->write('Update Data ' . $this->title . ' No Document : ' . $this->input->post('i_document') . ' i_supplier : ' . $this->input->post('i_supplier'). ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function delete()
	{
		...
		$i_bbr_id = $this->db->get_where('tm_bbr', ['i_company' => $this->i_company, 'i_bbr'  => $id])->row()->i_bbr_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_bbr_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_bbr_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
