...
class Alokasidnr extends CI_Controller
{
	public $id_menu = '51203';
	...
	public function add()
	{
		...
		$data = array(
			'data' 	 => $this->mymodel->get_data_dn($id)->row(),
			'dfrom'	 => $dfrom,
			'dto'	 => $dto,
			'i_supplier'	=> $i_supplier,
		);
	...
	public function delete2()
	{
		...
		$i_alokasi_id = $this->db->get_where('tm_alokasidn', ['i_company' => $this->i_company, 'i_alokasi'  => $id])->row()->i_alokasi_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
