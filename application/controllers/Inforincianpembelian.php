...
class Inforincianpembelian extends CI_Controller
{
    public $id_menu = '90205';
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('D' . $h, $this->lang->line('No Penerimaan'))
            ->setCellValue('E' . $h, $this->lang->line('Tgl Terima'))
            ->setCellValue('F' . $h, $this->lang->line('Nama Pemasok'))
            ->setCellValue('G' . $h, $this->lang->line('Kode Barang'))
            ->setCellValue('H' . $h, $this->lang->line('Nama Barang'))
            ->setCellValue('I' . $h, $this->lang->line('Terima'))
            ->setCellValue('J' . $h, $this->lang->line('Harga Rp'))
            ->setCellValue('K' . $h, $this->lang->line('Diskon Rp'))
            ->setCellValue('L' . $h, $this->lang->line('Total Rp'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('D' . $i, $row->i_gr_id)
                    ->setCellValue('E' . $i, Date::PHPToExcel($row->d_gr))
                    ->setCellValue('F' . $i, $row->e_supplier_name)
                    ->setCellValue('G' . $i, $row->i_product_id)
                    ->setCellValue('H' . $i, $row->e_product_name)
                    ->setCellValue('I' . $i, $row->kirim)
                    ->setCellValue('J' . $i, $row->jml)
                    ->setCellValue('K' . $i, $row->disc)
                    ->setCellValue('L' . $i, $row->tot);
