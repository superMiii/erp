...
class Kelompokharga extends CI_Controller
{
	public $id_menu = '20408';
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('margin', 'margin', 'trim|required|min_length[0]');
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$margin  = str_replace('_', '', str_replace('%', '', $this->input->post('margin', TRUE)));
				$this->mymodel->save($kode, $nama,$margin);
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $kode, $nama,$margin);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
