...
class Adjustment extends CI_Controller
{
	...
	public function add()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/extensions/sweetalert2.all.min.js',
				'app-assets/vendors/js/forms/select/select2.full.min.js',
				'assets/js/' . $this->folder . '/add.js?v=149',
			)
		);
	...
	public function get_area()
	{
		...
		$i_store_loc = $this->input->post('i_store_loc', TRUE);
		$d_so 		 = $this->input->post('d_so', TRUE);
		$area 		 = $this->mymodel->get_area($i_store_loc)->row();
		$query   = array(
			'detail' => $this->mymodel->get_area($i_store_loc)->result(),
			'detail_product' => $this->mymodel->get_product($i_store_loc, $area->i_store, $area->i_area, $d_so)->result(),
		);
	...
	public function get_stockopname()
	{
		...
		if ($i_store_loc != '' && $i_store != '' && $i_area != '') {
			$data   = $this->mymodel->get_stockopname($i_store_loc, $i_store, $i_area, $cari);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_stockopname,
					'text' => $row->i_stockopname_id . ' - ' . $row->d_stockopname,
				);
			}
		} else {
			$filter[] = array(
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('i_stockopname', 'i_stockopname', 'trim|required|min_length[0]');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan ' . $this->title . ' : ' . $this->input->post('i_document') . ' Store : ' . $this->input->post('i_store'). ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function edit()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/extensions/sweetalert2.all.min.js',
				'app-assets/vendors/js/forms/select/select2.full.min.js',
				'assets/js/' . $this->folder . '/edit.js?v=149',
			)
		);
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . ' : ' . $this->input->post('i_document') . ' Store : ' . $this->input->post('i_store'). ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function delete()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/extensions/sweetalert2.all.min.js',
				'app-assets/vendors/js/forms/select/select2.full.min.js',
				'assets/js/' . $this->folder . '/view.js?v=1995',
			)
		);
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_adjustment_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
