...
class Statuspelanggan extends CI_Controller
{
	public $id_menu = '20406';
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_customer_statusid = $this->db->get_where('tr_customer_status', ['i_company' => $this->i_company, 'i_customer_status'  => $id])->row()->i_customer_statusid;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_customer_statusid . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
