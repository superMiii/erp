...
class Alokasiknr extends CI_Controller
{
	public $id_menu = '51005';
	...
	public function add()
	{
		...
		$data = array(
			'data' 	 => $this->mymodel->get_data_kn($id)->row(),
			'dfrom'	 => $dfrom,
			'dto'	 => $dto,
			'harea'	 => $harea,
		);
	...
	public function delete2()
	{
		...
		$i_alokasi_id = $this->db->get_where('tm_alokasiknr', ['i_company' => $this->i_company, 'i_alokasi'  => $id])->row()->i_alokasi_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
