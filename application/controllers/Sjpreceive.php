...
class Sjpreceive extends CI_Controller
{
	public $id_menu = '41102';
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('d_receive', 'd_receive', 'trim|required|min_length[0]');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Receive ' . $this->title . ' No ID : ' . $this->input->post('id') . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
