...
class Girocair extends CI_Controller
{
	public $id_menu = '50402';
	...
	public function add()
	{
		...
		$this->logger->write('Membuka Form Add ' . $this->title);
	...
	public function delete2()
	{
		...
		$i_giro_id = $this->db->get_where('tm_giro', ['i_company' => $this->i_company, 'i_giro'  => $id])->row()->i_giro_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			$this->db->trans_begin();
			$this->mymodel->cancel($id,$alasan,$i_giro_id);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_giro_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
