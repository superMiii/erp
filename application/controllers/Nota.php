...
class Nota extends CI_Controller
{
	...
	public function add()
	{
		...
		$i_area = decrypt_url($this->uri->segment(6));
		$data = array(
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('d_nota', 'd_nota', 'trim|required|min_length[0]');
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_nota_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
