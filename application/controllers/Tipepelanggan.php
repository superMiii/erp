...
class Tipepelanggan extends CI_Controller
{
	public $id_menu = '20404';
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_customer_typeid = $this->db->get_where('tr_customer_type', ['i_company' => $this->i_company, 'i_customer_type'  => $id])->row()->i_customer_typeid;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_customer_typeid . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
