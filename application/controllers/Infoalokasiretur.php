...
class Infoalokasiretur extends CI_Controller
{
    public $id_menu = '90404';
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('E' . $h, $this->lang->line('No Alokasi'))
            ->setCellValue('F' . $h, $this->lang->line('Tgl Alokasi'))
            ->setCellValue('G' . $h, $this->lang->line('No Kredit Nota Retur'))
            ->setCellValue('H' . $h, $this->lang->line('Total Retur'))
            ->setCellValue('I' . $h, $this->lang->line('Lebih'))
            ->setCellValue('J' . $h, $this->lang->line('No Nota'))
            ->setCellValue('K' . $h, $this->lang->line('Tgl Nota'))
            ->setCellValue('L' . $h, $this->lang->line('Jumlah Nota'))
            ->setCellValue('M' . $h, $this->lang->line('Jumlah Nota'))
            ->setCellValue('N' . $h, $this->lang->line('Sisa Nota'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('E' . $i, $row->i_alokasi_id)
                    ->setCellValue('F' . $i, Date::PHPToExcel($row->d_alokasi))
                    ->setCellValue('G' . $i, $row->i_kn_id)
                    ->setCellValue('H' . $i, $row->v_jumlah)
                    ->setCellValue('I' . $i, $row->v_lebih)
                    ->setCellValue('J' . $i, $row->i_nota_id)
                    ->setCellValue('K' . $i, Date::PHPToExcel($row->d_nota))
                    ->setCellValue('L' . $i, $row->v_nota_netto)
                    ->setCellValue('M' . $i, $row->alok)
                    ->setCellValue('N' . $i, $row->v_sisa);
