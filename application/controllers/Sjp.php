...
class Sjp extends CI_Controller
{
	...
	public function save()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->trans_begin();
			$tgl = $this->input->post('d_document');
			$i_document = $this->mymodel->running_number(formatym2($tgl), formatY($tgl));
			$this->mymodel->save($tgl, $i_document);
	...
	public function delete()
	{
		...
		$i_gs_id = $this->db->get_where('tm_gs', ['i_company' => $this->i_company, 'i_gs'  => $id])->row()->i_gs_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_gs_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_gs_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
