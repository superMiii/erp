...
class Infoproduct extends CI_Controller
{
    public $id_menu = '90208';
    ...
    public function export()
    {
        ...
        /** Query Get Data */
        $query      = $this->mymodel->get_data($i_product_status);
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('E' . $h, $this->lang->line('Kategori Barang'))
            ->setCellValue('F' . $h, $this->lang->line('Motif Barang'))
            ->setCellValue('G' . $h, $this->lang->line('Status Barang'))
            ->setCellValue('H' . $h, $this->lang->line('Status'))
            ->setCellValue('I' . $h, $this->lang->line('Pareto'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    // ->setCellValue('B' . $i, Date::PHPToExcel($row->tgl))
                    ->setCellValue('B' . $i, $row->tgl)
                    ->setCellValue('C' . $i, $row->i_product_id)
                    ->setCellValue('D' . $i, $row->e_product_name)
                    ->setCellValue('E' . $i, $row->e_product_categoryname)
                    ->setCellValue('F' . $i, $row->e_product_motifname)
                    ->setCellValue('G' . $i, $row->e_product_statusname)
                    ->setCellValue('H' . $i, $row->act)
                    ->setCellValue('I' . $i, $row->prt);
