...
class Kumasuk extends CI_Controller
{
	public $id_menu = '50501';
	...
	public function get_dt()
	{
		...
		if (($i_area != '' || $i_area != null) && ($i_customer != '' || $i_customer != null)) {
			$data = $this->mymodel->get_dt(str_replace("'", "", $this->input->get('q')), $i_area, $i_customer);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_dt,
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('e_atasnama', 'e_atasnama', 'trim|required|min_length[0]');
	...
	public function delete2()
	{
		...
		$i_kum_id = $this->db->get_where('tm_kum', ['i_company' => $this->i_company, 'i_kum'  => $id])->row()->i_kum_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			$this->db->trans_begin();
			$this->mymodel->cancel($id,$alasan,$i_kum_id);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_kum_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
