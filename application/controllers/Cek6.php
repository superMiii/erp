...
class Cek6 extends CI_Controller
{
    public $id_menu = '115';
        ...
        if ($this->form_validation->run() == false) {
            $data = array(
				...
				$this->db->trans_begin();
                    ...
                    $this->mymodel->update_1($year, $month);
                    ...
                    if ($this->db->trans_status() === FALSE) {
                        ...
                        $this->logger->write('Clossing Stock : ' . $year . $month);
                        $data = array(
                            'sukses' => true,
                            'periode' => $year.$month,
                        );
        ...
        if ($this->form_validation->run() == false) {
            $data = array(
                ...
                'periode' => $year.$month,
                    ...
                    // $this->mymodel->update_periode($year, $month);
                    // $this->mymodel->update_log($year, $month);
                    $this->mymodel->update_2($year, $month); 
                    if ($this->db->trans_status() === FALSE) {
                        $this->db->trans_rollback();
                        $data = array(
                            'sukses' => false,
                        ...
                        $this->db->trans_commit();
                        $this->logger->write('Perbarui Clossing Stock : ' . $year . $month);
                        $data = array(
                            'sukses' => true,
                            'periode' => $year.$month,
                        );
    ...
    {        
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('A' . $h, "i_mutasi_saldoawal")
            ->setCellValue('B' . $h, "i_store")
            ->setCellValue('C' . $h, "Gudang")
            ->setCellValue('D' . $h, "i_periode")
            ->setCellValue('E' . $h, "i_product")
            ->setCellValue('F' . $h, "Kode")
            ->setCellValue('G' . $h, "Nama")
            ->setCellValue('H' . $h, "n_saldo_awal");
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ->setCellValue('A' . $i, $row->i_mutasi_saldoawal)
                    ->setCellValue('B' . $i, $row->i_store)
                    ->setCellValue('C' . $i, $row->e_store_name)
                    ->setCellValue('D' . $i, $row->i_periode)
                    ->setCellValue('E' . $i, $row->i_product)
                    ->setCellValue('F' . $i, $row->i_product_id)
                    ->setCellValue('G' . $i, $row->e_product_name)
                    ->setCellValue('H' . $i, $row->n_saldo_awal);
