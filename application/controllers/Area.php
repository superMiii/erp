...
class Area extends CI_Controller
{
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('istore', 'istore', 'trim|required|min_length[0]');
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$istore  = ucwords($this->input->post('istore', TRUE));
				$kode  = ucwords($this->input->post('kode', TRUE));
				$nama  = ucwords($this->input->post('nama', TRUE));
				$this->mymodel->save($kode, $nama, $istore);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan ' . $this->title . '  : ' . $kode . ' : ' . $istore . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $kode, $nama, $istore);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . '  : ' . $kode . ' : ' . $istore . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_area_id = $this->db->get_where('tr_area', ['i_company' => $this->i_company, 'i_area'  => $id])->row()->i_area_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_area_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
