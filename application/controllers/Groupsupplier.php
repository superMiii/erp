...
class Groupsupplier extends CI_Controller
{
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_supplier_groupid = $this->db->get_where('tr_supplier_group', ['i_company' => $this->i_company, 'i_supplier_group'  => $id])->row()->i_supplier_groupid;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_supplier_groupid . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
