...
class Jenispromo extends CI_Controller
{
	public $id_menu = '208';
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('i_promo_type_id', 'i_promo_type_id', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('e_promo_type_name', 'e_promo_type_name', 'trim|required|min_length[0]');
		$i_promo_type_id = strtoupper($this->input->post('i_promo_type_id', TRUE));
		$e_promo_type_name = ucwords(strtolower($this->input->post('e_promo_type_name', TRUE)));
		if ($this->form_validation->run() == false) {
			...
			$cek = $this->mymodel->cek($i_promo_type_id);
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->save($i_promo_type_id, $e_promo_type_name);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan ' . $this->title . ' : ' . $i_promo_type_id . ' : ' . $e_promo_type_name . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('i_promo_type_id', 'i_promo_type_id', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('i_promo_type_id_old', 'i_promo_type_id_old', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('e_promo_type_name', 'e_promo_type_name', 'trim|required|min_length[0]');
		$i_company_old = $this->input->post('i_company_old', TRUE);
		$i_promo_type = $this->input->post('i_promo_type', TRUE);
		$i_promo_type_id = strtoupper($this->input->post('i_promo_type_id', TRUE));
		$e_promo_type_name = ucwords(strtolower($this->input->post('e_promo_type_name', TRUE)));
		$i_promo_type_id_old = strtoupper($this->input->post('i_promo_type_id_old', TRUE));
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
				'ada'	 => false,
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($i_promo_type, $i_promo_type_id, $e_promo_type_name);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . ' : ' . $i_promo_type_id . ' : ' . $e_promo_type_name . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function changestatus()
	{
		...
		$i__id = $this->db->get_where('tr_promo_type', ['i_company' => $this->i_company, 'i_promo_type'  => $id])->row()->e_promo_type_name;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i__id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
