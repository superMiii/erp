...
class Pb extends CI_Controller
{
	...
	public function get_product()
	{
		...
		$data   = $this->mymodel->get_product($cari, $i_store);
		foreach ($data->result() as $row) {
			$filter[] = array(
				'id'   => $row->i_product,
	...
	public function get_product_detail()
	{
		...
		$query     = array(
			'detail' => $this->mymodel->get_product_detail($i_product, $i_store)->result()
		);
		echo json_encode($query);
	}
	...
	public function delete()
	{
		...
		$i_gs_id = $this->db->get_where('tm_gs2', ['i_company' => $this->i_company, 'i_gs'  => $id])->row()->i_gs_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
