...
class Infocon extends CI_Controller
{
    public $id_menu = '90313';
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'data'                  => $this->mymodel->get_data($dfrom, $dto, $i_product),
        );
    ...
    public function export()
    {
        ...
        /** Query Get Data */
        $query      = $this->mymodel->get_data($dfrom, $dto, $i_product);
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('A2', "Product : " . ucwords(strtolower($e_product_name)))
            ->setCellValue('A3', "Periode : $dfrom s/d $dto")
            ->setCellValue('A' . $h, $this->lang->line('No'))
            ->setCellValue('B' . $h, $this->lang->line('No. Konversi'))
            ->setCellValue('C' . $h, $this->lang->line('Tgl. Konversi'))
            ->setCellValue('D' . $h, $this->lang->line('Kode'))
            ->setCellValue('E' . $h, $this->lang->line('Barang Asal'))
            ->setCellValue('F' . $h, $this->lang->line('Jumlah Asal'))
            ->setCellValue('G' . $h, $this->lang->line('Kode Barang'))
            ->setCellValue('H' . $h, $this->lang->line('Nama Barang'))
            ->setCellValue('I' . $h, $this->lang->line('Jml Konversi'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C' . $i, Date::PHPToExcel($row->tgl))
                    ->setCellValue('D' . $i, $row->p1)
                    ->setCellValue('E' . $i, $row->p2)
                    ->setCellValue('F' . $i, $row->p3)
                    ->setCellValue('G' . $i, $row->q1)
                    ->setCellValue('H' . $i, $row->q2)
                    ->setCellValue('I' . $i, $row->q3);
