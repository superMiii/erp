...
class Infokbpv extends CI_Controller
{
    public $id_menu = '90428';
    ...
    public function index()
    {
        ...
        if ($i_coa != 'ALL') {
            $e_coa_name = $this->db->get_where('tr_coa', ['f_kas_besar' => true, 'i_company' => $this->i_company, 'i_coa' => $i_coa])->row()->e_coa_name;
        } else {
            $e_coa_name = '( Semua )';
        }
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('B' . $i, $row->i_pv_id)
                    ->setCellValue('C' . $i, Date::PHPToExcel($row->d_pv))
                    ->setCellValue('D' . $i, $row->e_area_name)
                    ->setCellValue('E' . $i, $row->e_coa_name)
                    ->setCellValue('F' . $i, $row->e_remark)
                    ->setCellValue('G' . $i, $row->v_pv);
