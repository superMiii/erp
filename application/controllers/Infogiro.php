...
class Infogiro extends CI_Controller
{
    public $id_menu = '90401';
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('B' . $h, $this->lang->line('Nomor Dokumen'))
            ->setCellValue('C' . $h, $this->lang->line('Tanggal Dokumen'))
            ...
            ->setCellValue('G' . $h, $this->lang->line('Terima'))
            ->setCellValue('H' . $h, $this->lang->line('Tanggal Cair'))
            ->setCellValue('I' . $h, $this->lang->line('Tanggal Tolak'))
            ->setCellValue('J' . $h, $this->lang->line('Jatuh Tempo'))
            ->setCellValue('K' . $h, $this->lang->line('Bank'))
            ->setCellValue('L' . $h, $this->lang->line('Jumlah'))
            ->setCellValue('M' . $h, $this->lang->line('Keterangan'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('B' . $i, $row->i_giro_id)
                    ->setCellValue('C' . $i, $row->d_giro)
                    ->setCellValue('D' . $i, $row->e_area_name)
                    ->setCellValue('E' . $i, $row->codee)
                    ->setCellValue('F' . $i, $row->e_customer)
                    ->setCellValue('G' . $i, Date::PHPToExcel($row->terima))
                    ->setCellValue('H' . $i, Date::PHPToExcel($row->cair))
                    ->setCellValue('I' . $i, Date::PHPToExcel($row->tolak))
                    ->setCellValue('J' . $i, $row->tempo)
                    ->setCellValue('K' . $i, $row->bank)
                    ->setCellValue('L' . $i, $row->jml)
                    ->setCellValue('M' . $i, $row->e_giro_description);
