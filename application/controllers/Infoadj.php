...
class Infoadj extends CI_Controller
{
    public $id_menu = '90316';
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('B' . $i, $row->i_adjustment_id)
                    ->setCellValue('C' . $i, Date::PHPToExcel($row->d_adjustment))
                    ->setCellValue('D' . $i, $row->e_store_name)
                    ->setCellValue('E' . $i, $row->e_remark)
                    ->setCellValue('F' . $i, $row->r2)
                    ->setCellValue('G' . $i, $row->i_product_id)
                    ->setCellValue('H' . $i, $row->e_product_name)
                    ->setCellValue('I' . $i, $row->n_adjustment);
