...
class Inforetur extends CI_Controller
{
    public $id_menu = '90113';
    ...
    public function view()
    {
        ...
        $i_ttb          = decrypt_url($this->uri->segment(3));
        ...
        $i_ttb_id = $this->db->get_where('tm_ttbretur', ['i_ttb' => $i_ttb])->row()->i_ttb_id;
        $data = array(
            'detail'           => $this->mymodel->get_data_detail($i_ttb),
            'dfrom'            => $dfrom,
            'dto'              => $dto,
            'i_area'           => $i_area,
            'e_area_name'      => ucwords(strtolower($e_area_name)),
            'i_ttb_id'         => $i_ttb_id,
            // 'i_ttb'    => $this->logger->get_product($i_ttb), 
        );
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('F' . $h, $this->lang->line('Nama Pramuniaga'))
            ->setCellValue('G' . $h, $this->lang->line('PPN'))
            ->setCellValue('H' . $h, $this->lang->line('Jumlah TTB'))
            ->setCellValue('I' . $h, $this->lang->line('Alasan Retur'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C' . $i, $row->d_ttb)
                    ->setCellValue('D' . $i, $row->e_area_name)
                    ->setCellValue('E' . $i, $row->e_customer_name)
                    ->setCellValue('F' . $i, $row->e_salesman_name)
                    ->setCellValue('G' . $i, $row->f_ttb_pkp)
                    ->setCellValue('H' . $i, $row->v_ttb_netto)
                    ->setCellValue('I' . $i, $row->e_alasan_retur_name);
