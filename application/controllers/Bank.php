...
class Bank extends CI_Controller
{
	...
	public function save()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$norek  = ucwords($this->input->post('norek', TRUE));
				$namarek  = ucwords($this->input->post('namarek', TRUE));
				$this->mymodel->save($kode, $nama, $norek, $namarek);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan Data ' . $this->title . ' : ' . $kode . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $kode, $nama, $norek, $namarek);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Update Data ' . $this->title . ' : ' . $kode . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_bank_id = $this->db->get_where('tr_bank', ['i_company' => $this->i_company, 'i_bank'  => $id])->row()->i_bank_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_bank_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
