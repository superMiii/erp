...
class Hargajualrumus extends CI_Controller
{
	public $id_menu = '20112';
	...
	public function add()
	{
		...
		$data = array(
			'grade'	=> $this->db->get_where('tr_product_grade', ['f_product_gradeactive' => true, 'i_company' => $this->i_company]),
		);
	...
	public function get_price_group()
	{
		...
		$query  = array(
			'detail' => $this->mymodel->get_price_group($i_product, $i_product_grade)->result_array(),
		);
	...
	public function save()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			$this->db->trans_begin();
			$this->mymodel->save($i_product, $i_product_grade);
			if ($this->db->trans_status() === FALSE) {
				...
				$this->logger->write('Simpan ' . $this->title . ' : i_product =' . $i_product . ', i_product_grade = ' . $i_product_grade . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
					'ada'	 => false,
				);
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $isuppliergroup, $kode, $nama);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
