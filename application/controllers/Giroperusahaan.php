...
class Giroperusahaan extends CI_Controller
{
	public $id_menu = '50403';
	...
	public function delete2()
	{
		...
		$i_giro_id = $this->db->get_where('tm_giro_company', ['i_company' => $this->i_company, 'i_giro'  => $id])->row()->i_giro_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Cancel Data ' . $this->title . ' Id : ' . $id . ' : ' . $i_giro_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
