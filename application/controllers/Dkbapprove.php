...
class Dkbapprove extends CI_Controller
{
	public $id_menu = '409';
	...
	public function index()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/forms/select/select2.full.min.js?v=1',
			)
		);
	...
	public function get_do()
	{
		...
		if ($i_area != '' || $i_area != null) {
			$data = $this->mymodel->get_do($cari, $i_area);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_do,
					'text' => $row->i_do_id,
	...
	public function delete()
	{
		...
		$i_sl_id = $this->db->get_where('tm_sl', ['i_company' => $this->i_company, 'i_sl'  => $id])->row()->i_sl_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_sl_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_sl_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function approve()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Approve Data ' . $this->title . ' Id : ' . $id . ' Note : ' . $note);
				$data = array(
					'sukses' => true,
				);
			}
