...
class Infocollc extends CI_Controller
{
    public $id_menu = '90414';
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('F' . $h, $this->lang->line('Kode Pelanggan'))
            ->setCellValue('G' . $h, $this->lang->line('Nama Pelanggan'))
            ->setCellValue('H' . $h, $this->lang->line('TOP'))
            ->setCellValue('I' . $h, $this->lang->line('Toleransi'))
            ->setCellValue('J' . $h, $this->lang->line('Tgl Jatuh Tempo'))
            ->setCellValue('K' . $h, $this->lang->line('Lama'))
            ->setCellValue('L' . $h, $this->lang->line('Jumlah Nota'))
            ->setCellValue('M' . $h, $this->lang->line('Target'))
            ->setCellValue('N' . $h, $this->lang->line('Realisasi'))
            ->setCellValue('O' . $h, $this->lang->line('Persentase'))
            ->setCellValue('P' . $h, $this->lang->line('Keterangan'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('F' . $i, $row->i_customer_id)
                    ->setCellValue('G' . $i, $row->e_customer_name)
                    ->setCellValue('H' . $i, $row->n_customer_top)
                    ->setCellValue('I' . $i, $row->n_toleransi)
                    ->setCellValue('J' . $i, Date::PHPToExcel($row->d_jatuh_tempo))
                    ->setCellValue('K' . $i, $row->lama)
                    ->setCellValue('L' . $i, $row->v_nota_netto)
                    ->setCellValue('M' . $i, $row->v_target)
                    ->setCellValue('N' . $i, $row->v_realisasi)
                    ->setCellValue('O' . $i, $row->persen)
                    ->setCellValue('P' . $i, $row->e_remark);
            ...
            $spreadsheet->getActiveSheet()->setCellValue('O' . $i, "=(SUM(N" . $x . ":N" . ($i - 1) . ") / SUM(M" . $x . ":M" . ($i - 1) . "))*100");
            $spreadsheet->getActiveSheet()
                ->getStyle('L' . $x . ':N' . $i)
                ->getNumberFormat()
