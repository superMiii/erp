...
class Infosjnota extends CI_Controller
{
    public $id_menu = '90411';
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('E' . $h, $this->lang->line('Tgl SJ'))
            ->setCellValue('F' . $h, $this->lang->line('Jumlah SJ'))
            ->setCellValue('G' . $h, $this->lang->line('No Nota'))
            ->setCellValue('H' . $h, $this->lang->line('Tgl Nota'))
            ->setCellValue('I' . $h, $this->lang->line('Nilai Kotor'))
            ->setCellValue('J' . $h, $this->lang->line('PPN'))
            ->setCellValue('K' . $h, $this->lang->line('Diskon'))
            ->setCellValue('L' . $h, $this->lang->line('Total Nota'))
            ->setCellValue('M' . $h, $this->lang->line('Sisa'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C' . $i, $row->customer)
                    ->setCellValue('D' . $i, $row->i_do_id)
                    ->setCellValue('E' . $i, Date::PHPToExcel($row->d_do))
                    ->setCellValue('F' . $i, $row->v_do)
                    ->setCellValue('G' . $i, $row->i_nota_id)
                    ->setCellValue('H' . $i, Date::PHPToExcel($row->d_nota))
                    ->setCellValue('I' . $i, $row->v_nota_gross)
                    ->setCellValue('J' . $i, $row->v_nota_ppn)
                    ->setCellValue('K' . $i, $row->v_nota_discount)
                    ->setCellValue('L' . $i, $row->v_nota_netto)
                    ->setCellValue('M' . $i, $row->v_sisa);
