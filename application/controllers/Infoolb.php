...
class Infoolb extends CI_Controller
{
    public $id_menu = '90430';
    ...
    public function index()
    {
        ...
        if ($i_coa != 'ALL') {
            $e_coa_name = $this->db->get_where('tr_coa', ['i_company' => $this->i_company, 'i_coa' => $i_coa])->row()->e_coa_name;
        } else {
            $e_coa_name = 'Semua';
        }
        $data = array(
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('A4', "Bank : $e_coa_name")
            ->setCellValue('A' . $h, $this->lang->line('No'))
            ->setCellValue('B' . $h, $this->lang->line('Tgl Dokumen'))
            ->setCellValue('C' . $h, $this->lang->line('Nama Bank'))
            ->setCellValue('D' . $h, $this->lang->line('No Dokumen'))
            ->setCellValue('E' . $h, $this->lang->line('Nama Area Provinsi'))
            ->setCellValue('F' . $h, $this->lang->line('TF/GR/TN'))
            ->setCellValue('G' . $h, $this->lang->line('Sisa'))
            ->setCellValue('H' . $h, $this->lang->line('Keterangan'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('B' . $i, Date::PHPToExcel($row->d_bukti))
                    ->setCellValue('C' . $i, $row->e_coa_name)
                    ->setCellValue('D' . $i, $row->i_rv_id)
                    ->setCellValue('E' . $i, $row->e_area_name)
                    ->setCellValue('F' . $i, $row->e_rv_refference_type_name)
                    ->setCellValue('G' . $i, $row->v_rv_saldo)
                    ->setCellValue('H' . $i, $row->e_remark);
