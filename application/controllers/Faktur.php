...
class Faktur extends CI_Controller
{
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('kode', 'kode', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('n_year', 'n_year', 'trim|required|min_length[0]');
		...
		$n_year 	= $this->input->post('n_year', TRUE);
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $kode, $n_year, $n_start, $n_end);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_pajak_id = $this->db->get_where('tr_pajak', ['i_company' => $this->i_company, 'i_pajak'  => $id])->row()->i_pajak_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_pajak_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
