...
class Salesbyspbvssj extends CI_Controller
{
    public $id_menu = '70113';
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('B' . $h, $this->lang->line('Nama Pelanggan'))
            ->setCellValue('C' . $h, $this->lang->line('Status'))
            ->setCellValue('D' . $h, $this->lang->line('Nama Area Provinsi'))
            ->setCellValue('E' . $h, $this->lang->line('Kota'))
            ...
            ->setCellValue('J' . $h, str_replace('\r\n', "<br>", $this->lang->line('SPBApp1')))
            ->setCellValue('K' . $h, str_replace('\r\n', "<br>", $this->lang->line('SPBApp2')))
            ->setCellValue('L' . $h, $this->lang->line('No SJ'))
            ->setCellValue('M' . $h, $this->lang->line('Tgl SJ'))
            ->setCellValue('N' . $h, str_replace('\r\n', "<br>", $this->lang->line('SPBSJ')))
            ->setCellValue('O' . $h, $this->lang->line('No Nota'))
            ->setCellValue('P' . $h, $this->lang->line('Tgl Nota'))
            ->setCellValue('Q' . $h, str_replace('\r\n', "<br>", $this->lang->line('SPBNota')))
            ->setCellValue('R' . $h, $this->lang->line('Nilai Pesanan'))
            ->setCellValue('S' . $h, $this->lang->line('Nilai Pemenuhan'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('B' . $i, $row->cus)
                    ->setCellValue('C' . $i, $row->e_status_so_name)
                    ->setCellValue('D' . $i, $row->araa)
                    ->setCellValue('E' . $i, $row->e_city_name)
                    ->setCellValue('F' . $i, $row->i_so_id)
                    ->setCellValue('G' . $i, $row->d_so)
                    ->setCellValue('H' . $i, $row->d_approve1)
                    ->setCellValue('I' . $i, $row->d_approve2)
                    ->setCellValue('J' . $i, $row->d1)
                    ->setCellValue('K' . $i, $row->d2)
                    ->setCellValue('L' . $i, $row->i_do_id)
                    ->setCellValue('M' . $i, $row->d_do)
                    ->setCellValue('N' . $i, $row->d3)
                    ->setCellValue('O' . $i, $row->i_nota_id)
                    ->setCellValue('P' . $i, $row->d_nota)
                    ->setCellValue('Q' . $i, $row->d4)
                    ->setCellValue('R' . $i, $row->v_so)
                    ->setCellValue('S' . $i, $row->v_nota_netto);
