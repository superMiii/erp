...
class Rv extends CI_Controller
{
	...
	public function index()
	{
		...
		$i_type = $this->input->post('i_rv_type', TRUE);
		if ($i_type == '') {
	...
	public function number()
	{
		...
		$i_coa = $this->input->post('i_coa', TRUE);
		$number = '';
		if ($tanggal != '' && $i_area != '' && $i_rv_type != '' && $i_coa != '') {
			$number = $this->mymodel->running_number(date('ym', strtotime($tanggal)), date('Y', strtotime($tanggal)), date('m', strtotime($tanggal)), $i_rv_type, $i_area,$i_coa);
		}
	...
	public function suzu()
	{
		...
		if ($tanggal != '' && $i_area != '' && $i_rv_type != '' && $i_coa != '') {
			$query  = array(
				'ori' => $this->mymodel->suzu(date('ym', strtotime($tanggal)), date('Y', strtotime($tanggal)), date('m', strtotime($tanggal)), $i_rv_type, $i_area, $i_coa)->result_array()
			);
		}
	...
	public function get_rv_type()
	{
		...
		foreach ($data->result() as $row) {
			$filter[] = array(
				'id'   => $row->i_rv_type,
				'text' => $row->i_rv_type_id . ' - ' . $row->e_rv_type_name,
			);
		}
		echo json_encode($filter);
	}
	...
	public function get_coa()
	{
		...
		if ($i_rv_type != '') {
			$data = $this->mymodel->get_coa(str_replace("'", "", $this->input->get('q')), $i_rv_type);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_coa,
	...
	public function get_coa2()
	{
		...
		$i_rv_type = $this->input->get('i_rv_type');
		if ($i_rv_type != '') {
			$data = $this->mymodel->get_coa2($cari, $i_rv_type);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_coa,
					'text' => $row->i_coa_id . ' - ' . $row->e_coa_name,
	...
	public function get_coa_type()
	{
		...
		$i_rv_type = $this->input->get('i_rv_type');
		if ($i_rv_type != '') {
			$data = $this->mymodel->get_coa_type(str_replace("'", "", $this->input->get('q')), $i_rv_type);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_coa,
					'text' => $row->i_coa_id . ' - ' . $row->e_coa_name,
	...
	public function get_reference()
	{
		...
		if ($i_rv_refference_type != '' || $i_rv_refference_type != null) {
			$data = $this->mymodel->get_reference($cari, $i_rv_refference_type);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->id,
					'text' => $row->kode . ' #(' . $row->v_jumlah . ') ' . ' #' . $row->ara . ' ',
				);
			}
		} else {
			$filter[] = array(
	...
	public function get_detail_referensi()
	{
		...
		if (($id != '' || $id != null) && ($i_rv_refference_type != '' || $i_rv_refference_type != null)) {
			...
			// }
			$query  = array(
				'detail' => $this->mymodel->get_reference_detail($id, $i_rv_refference_type)->result_array()
			);
		}
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('v_rv', 'v_rv', 'trim|required|min_length[0]');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . ' No Dokumen : ' . $this->input->post('i_document') . ' i_coa : ' . $this->input->post('i_coa') . ' v_rv : ' . $this->input->post('v_rv') . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function delete2()
	{
		...
		$i_rv_id = $this->db->get_where('tm_rv', ['i_company' => $this->i_company, 'i_rv'  => $id])->row()->i_rv_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			$this->db->trans_begin();
			$this->mymodel->cancel($id,$alasan,$i_rv_id);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_rv_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function get_tex()
	{
		...
		$data = $this->mymodel->get_tex($i_rv_type);
		if ($data->num_rows() > 0) {
			echo json_encode($data->row()->i_rv_type_id);
		} else {
			echo json_encode(0);
		}
	}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_rv_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
