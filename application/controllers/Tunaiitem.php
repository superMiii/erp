...
class Tunaiitem extends CI_Controller
{
	public $id_menu = '506';
	...
	public function delete2()
	{
		...
		$i_tunai_id = $this->db->get_where('tm_tunai', ['i_company' => $this->i_company, 'i_tunai'  => $id])->row()->i_tunai_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			$this->db->trans_begin();
			$this->mymodel->cancel($id,$alasan,$i_tunai_id);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_tunai_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
