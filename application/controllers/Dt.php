...
class Dt extends CI_Controller
{
	...
	public function get_nota()
	{
		...
		if ($i_area != '' || $i_area != null) {
			$data = $this->mymodel->get_nota($cari, $i_area);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_nota,
					'text' => $row->i_nota_id . ' - ' . $row->e_customer,
				);
			}
		} else {
			$filter[] = array(
	...
	public function get_detail_nota()
	{
		...
		$query  = array(
			'detail' => $this->mymodel->get_detail_nota($id)->result_array()
		);
	...
	public function delete()
	{
		...
		$this->form_validation->set_rules('id', 'id', 'trim|required|min_length[0]');
		$id = $this->input->post('id', TRUE);
		$i_dt_id = $this->db->get_where('tm_dt', ['i_company' => $this->i_company, 'i_dt'  => $id])->row()->i_dt_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_dt_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_dt_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
