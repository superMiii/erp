...
class Tingkatpelanggan extends CI_Controller
{
	public $id_menu = '20405';
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_customer_levelid = $this->db->get_where('tr_customer_level', ['i_company' => $this->i_company, 'i_customer_level'  => $id])->row()->i_customer_levelid;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_customer_levelid . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
