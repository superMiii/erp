...
class Bbmretur extends CI_Controller
{
	public $id_menu = '410	';
	...
	public function get_product_detail()
	{
		...
		$query  = array(
			'detail' => $this->mymodel->get_product_detail($i_product)->result_array()
		);
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('i_ttb', 'i_ttb', 'trim|required|min_length[0]');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . ' No Document : ' . $this->input->post('i_document') . ' I_Area : ' . $this->input->post('i_area') . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function delete()
	{
		...
		$i_bbm_id = $this->db->get_where('tm_bbm', ['i_company' => $this->i_company, 'i_bbm'  => $id])->row()->i_bbm_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_bbm_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
