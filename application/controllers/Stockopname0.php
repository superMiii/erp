...
class Stockopname0 extends CI_Controller
{
	public $i_menu = '415';
	...
	public function add()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/extensions/sweetalert2.all.min.js',
				'app-assets/vendors/js/forms/select/select2.full.min.js',
				'assets/js/' . $this->folder . '/add.js?v=14',
			)
		);
	...
	public function get_store2()
	{
		...
		$query   = array(
			'detail' => $this->mymodel->get_store3($i_store_loc)->row(),
			'detail_product' => $this->mymodel->get_product($i_store_loc, $i_area, $d_so)->result(),
		);
		echo json_encode($query);
	}

	/** Get Area */
	public function get_areaku()
	{
		$filter = [];
		$cari = str_replace("'", "", $this->input->get('q'));
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan Data ' . $this->title . ' ID : ' . $this->input->post('i_document'));
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function edit()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/extensions/sweetalert2.all.min.js',
				'app-assets/vendors/js/forms/select/select2.full.min.js',
				'assets/js/' . $this->folder . '/edit.js?v=14',
			)
		);
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$this->logger->write('Update Data ' . $this->title . ' No Document : ' . $this->input->post('i_document') . ' Store : ' . $this->input->post('i_store'));
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function view()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/extensions/sweetalert2.all.min.js',
				'app-assets/vendors/js/forms/select/select2.full.min.js',
				'assets/js/' . $this->folder . '/view.js?v=199',
			)
		);
