...
class Rrkh extends CI_Controller
{
	public $id_menu = '306';
	...
	{
		$filter = [];
		$data = $this->mymodel->get_kunjungan(str_replace("'", "", $this->input->get('q')));
		foreach ($data->result() as $row) {
			$filter[] = array(
				'id'   => $row->i_kunjungan_type,
				'text' => $row->e_kunjungan_type_name,
			);
		}
		echo json_encode($filter);
	}
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan ' . $this->title . ' Tgl Dokumen : ' . $this->input->post('d_document') . ' I_Area : ' . $this->input->post('i_area') . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('i_salesman', 'i_salesman', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('d_document', 'd_document', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('d_document_old', 'd_document_old', 'trim|required|min_length[0]');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . ' Tgl Dokumen : ' . $this->input->post('d_document') . ' I_Area : ' . $this->input->post('i_area') . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function delete()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $d_rrkh . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
