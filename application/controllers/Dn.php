...
class Dn extends CI_Controller
{
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('d_refference', 'd_refference', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('e_remark', 'e_remark', 'trim|required|min_length[0]');
	...
	public function delete2()
	{
		...
		$i_dn_id = $this->db->get_where('tm_dn', ['i_company' => $this->i_company, 'i_dn'  => $id])->row()->i_dn_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			$this->db->trans_begin();
			$this->mymodel->cancel($id,$alasan,$i_dn_id);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_commit();
				$this->logger->write('Cancel Data ' . $this->title . ' Id : ' . $id . ' : ' . $i_dn_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
