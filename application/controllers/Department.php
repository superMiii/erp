...
class Department extends CI_Controller
{
	...
	public function save()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->mymodel->save($kode, $nama, $deskripsi);
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $kode, $nama,$deskripsi);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_department_id = $this->db->get_where('tr_department', ['i_company' => $this->i_company, 'i_department'  => $id])->row()->i_department_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_department_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
