...
class Sj2 extends CI_Controller
{
	public $id_menu = '415';
	...
	public function indexx()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/pickers/pickadate/picker.js',
				'app-assets/vendors/js/pickers/pickadate/picker.date.js',
				// 'assets/js/' . $this->folder . '/indexx.js',
	...
	public function delete2()
	{
		...
		$i_do_id = $this->db->get_where('tm_do', ['i_company' => $this->i_company, 'i_do'  => $id])->row()->i_do_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			$this->db->trans_begin();
			$this->mymodel->cancel($id,$alasan,$i_do_id);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_do_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_do_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
