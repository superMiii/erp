...
class Returpenjualan extends CI_Controller
{
	...
	public function get_product()
	{
		...
		$i_customer = $this->input->get('i_customer');
		if ($i_customer != '') {
			$data = $this->mymodel->get_product($cari, $i_customer);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_nota_item,
					'text' => $row->i_product_id . ' - ' . $row->e_product_name . ' - ( ' . $row->i_nota_id . ' )',
				);
			}
		} else {
			$filter[] = array(
	...
	{
		header("Content-Type: application/json", true);
		$i_nota_item = $this->input->post('i_nota_item', TRUE);
		$i_customer  = $this->input->post('i_customer', TRUE);
		$query  = array(
			'detail' => $this->mymodel->get_product_price($i_nota_item, $i_customer)->result_array()
		);
		echo json_encode($query);
	}
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('i_alasan_retur', 'i_alasan_retur', 'trim|required|min_length[0]');
	...
	public function delete()
	{
		...
		$i_ttb_id = $this->db->get_where('tm_ttbretur', ['i_company' => $this->i_company, 'i_ttb'  => $id])->row()->i_ttb_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_ttb_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
