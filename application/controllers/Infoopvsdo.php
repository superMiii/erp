...
class Infoopvsdo extends CI_Controller
{
    public $id_menu = '90202';
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'data' => $this->mymodel->get_data($dfrom, $dto, $i_supplier),
        );
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('A3', "Supplier : $e_supplier_name")
            ->setCellValue('A' . $h, $this->lang->line('No'))
            ->setCellValue('B' . $h, $this->lang->line('Kode Barang'))
            ->setCellValue('C' . $h, $this->lang->line('Nama Barang'))
            ->setCellValue('D' . $h, $this->lang->line('Qty OP'))
            ->setCellValue('E' . $h, $this->lang->line('Rp OP'))
            ->setCellValue('F' . $h, $this->lang->line('Qty DO'))
            ->setCellValue('G' . $h, $this->lang->line('Rp DO'))
            // ->setCellValue('H' . $h, $this->lang->line('Persen Qty'))
            // ->setCellValue('I' . $h, $this->lang->line('Persen Rp'))
            ->setCellValue('H' . $h, $this->lang->line('Nama Pemasok'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('D' . $i, $row->n_order)
                    ->setCellValue('E' . $i, $row->v_po)
                    ->setCellValue('F' . $i, $row->n_delivery)
                    ->setCellValue('G' . $i, $row->v_gr)
                    // ->setCellValue('H' . $i, $row->persen_qty)
                    // ->setCellValue('I' . $i, $row->persen_rp)
                    ->setCellValue('H' . $i, $row->e_supplier_name);
            ...
            // $spreadsheet->getActiveSheet()->setCellValue('H' . $i, "=(F" . $i . "/D" . $i . ")");
            // $spreadsheet->getActiveSheet()->setCellValue('I' . $i, "=(G" . $i . "/E" . $i . ")");
