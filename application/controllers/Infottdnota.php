...
class Infottdnota extends CI_Controller
{
    public $id_menu = '90407';
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('E' . $h, $this->lang->line('Nama Pramuniaga'))
            ->setCellValue('F' . $h, $this->lang->line('No SJ'))
            ->setCellValue('G' . $h, $this->lang->line('Tgl SJ'))
            ->setCellValue('H' . $h, $this->lang->line('No Nota'))
            ->setCellValue('I' . $h, $this->lang->line('Tgl Nota'))
            ->setCellValue('J' . $h, $this->lang->line('Jumlah'))
            ->setCellValue('K' . $h, $this->lang->line('Sisa'))
            ->setCellValue('L' . $h, $this->lang->line('Pemenuhan'))
            ->setCellValue('M' . $h, $this->lang->line('Cetak'))
            ->setCellValue('N' . $h, $this->lang->line('Remark'));
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('F' . $i, $row->i_do_id)
                    ->setCellValue('G' . $i, Date::PHPToExcel($row->d_do))
                    ->setCellValue('H' . $i, $row->i_nota_id)
                    ->setCellValue('I' . $i, Date::PHPToExcel($row->d_nota))
                    ->setCellValue('J' . $i, $row->v_nota_netto)
                    ->setCellValue('K' . $i, $row->v_sisa)
                    ->setCellValue('L' . $i, $row->pnh)
                    ->setCellValue('M' . $i, $row->nprint);
            ...
            $spreadsheet->getActiveSheet()->mergeCells('L' . $i . ':N' . $i);
