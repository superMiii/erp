...
class Spbpromo extends CI_Controller
{
	public $id_menu = '30702';
	...
	public function get_promo()
	{
		...
		if ($tanggal != '') {
			$data = $this->mymodel->get_promo(str_replace("'", "", $this->input->get('q')), $tanggal);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_promo,
					'text' => $row->i_promo_id . ' - ' . $row->e_promo_name,
				);
			}
		}
	...
	public function get_promo_detail()
	{
		...
		$query  = array(
			'promo' => $this->mymodel->get_promo_detail($ipromo)->result_array()
		);
	...
	public function get_area()
	{
		...
		if ($i_promo != '' && $f_all_area != '') {
			$data = $this->mymodel->get_area(str_replace("'", "", $this->input->get('q')), $i_promo, $f_all_area);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_area,
					'text' => $row->i_area_id . ' - ' . $row->e_area_name,
			...
			$filter[] = array(
				'id'   => null,
				'text' => $this->lang->line('Pilih') . ' ' . $this->lang->line('Promo'),
			);
		}
		echo json_encode($filter);
	}
	...
	public function get_customer()
	{
		...
		if ($i_area != '' && $i_promo != '') {
			$data = $this->mymodel->get_customer($cari, $i_area, $i_promo, $f_all_customer);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_customer,
			...
			$filter[] = array(
				'id'   => null,
				'text' => $this->lang->line('Pilih') . ' ' . $this->lang->line('Area') . ' dan ' . $this->lang->line('Promo'),
			);
		}
		echo json_encode($filter);
	}
	...
	public function get_customer_detail()
	{
		...
		$ipromoid  = $this->input->post('i_promo', TRUE);
		$query  = array(
			'header' => $this->mymodel->get_customer_detail($icustomer, $ipromoid)->result_array()
		);
		echo json_encode($query);
	}
	...
	public function get_product()
	{
		...
		if ($i_price_group != '' && $i_product_group != '' && $i_promo != '') {
			$data = $this->mymodel->get_product($cari, $i_product_group, $i_price_group, $i_promo, $f_all_product);
			foreach ($data->result() as $row) {
				$filter[] = array(
					'id'   => $row->i_product,
			...
			$filter[] = array(
				'id'   => null,
				'text' => $this->lang->line('Pilih') . ' ' . $this->lang->line('Kelompok Barang') . ' / ' . $this->lang->line('Pelanggan') . ' / ' . $this->lang->line('Promo'),
			);
		}
		echo json_encode($filter);
	}
	...
	public function get_product_price()
	{
		...
		$query  = array(
			'detail' => $this->mymodel->get_product_price($i_price_group, $i_product_group, $i_product, $i_promo, $f_all_product)->result_array()
		);
		echo json_encode($query);
	}
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('i_promo', 'i_promo', 'trim|required|min_length[0]');
	...
	public function delete()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_so_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_so_id. ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
