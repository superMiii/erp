...
class Knretur extends CI_Controller
{
	public $id_menu = '51101';
	...
	public function add()
	{
		...
		$data = array(
			'data' 	 => $this->mymodel->get_data_referensi($id)->row(),
			'detail' => $this->mymodel->get_data_detail_referensi($id),
			'dfrom'	 => $dfrom,
			'dto'	 => $dto,
			'harea'	 => $harea,
		);
	...
	public function get_pajak()
	{
		...
		$data = $this->mymodel->get_pajak(str_replace("'", "", $this->input->get('q')), $i_customer);
		foreach ($data->result() as $row) {
			$filter[] = array(
				'id'   => $row->i_seri_pajak,
				'text' => $row->i_nota_id . ' - ' . $row->i_seri_pajak,
			);
		}
		echo json_encode($filter);
	}
	...
	/** Get Detail Pajak */
	public function detail_pajak()
	{
		$i_customer = $this->input->post('i_customer', TRUE);
		$i_seri_pajak = $this->input->post('i_seri_pajak', TRUE);
		if ($i_seri_pajak != '') {
			$tanggal = $this->mymodel->get_detail_pajak($i_customer, $i_seri_pajak)->row()->d_pajak;
		} else {
			$tanggal = '';
		}
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('i_refference', 'i_refference', 'trim|required|min_length[0]');
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan ' . $this->title . ' No Dokumen : ' . $this->input->post('i_document') . ' I_Area : ' . $this->input->post('i_area') . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . ' No Dokumen : ' . $this->input->post('i_document') . ' I_Area : ' . $this->input->post('i_area') . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function delete2()
	{
		...
		$i_kn_id = $this->db->get_where('tm_kn', ['i_company' => $this->i_company, 'i_kn'  => $id])->row()->i_kn_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			$this->db->trans_begin();
			$this->mymodel->cancel($id,$alasan,$i_kn_id);
			if ($this->db->trans_status() === FALSE) {
				$this->db->trans_rollback();
				$data = array(
					'sukses' => false,
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_kn_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function view()
	{
		...
		add_js(
			array(
				...
				'app-assets/vendors/js/forms/select/select2.full.min.js',
				'app-assets/vendors/js/forms/toggle/bootstrap-checkbox.min.js',
				'assets/js/' . $this->folder . '/view.js?v=31917410',
			)
		);
	...
	public function print()
	{
		...
		$x = decrypt_url($this->uri->segment(3));
		$id  = explode('|', $x)[0];
		$cek = explode('|', $x)[1];
		$data = array(
			'data' 	 => $this->mymodel->get_data($id)->row(),
		...
		if ($cek == 't') {
			$this->load->view($this->folder . '/vformprt', $data);
		} else {
