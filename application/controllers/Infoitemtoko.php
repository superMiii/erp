...
class Infoitemtoko extends CI_Controller
{
    public $id_menu = '90422';
    ...
    public function index()
    {
        ...
        $data = array(
            ...
            'data' => $this->mymodel->get_data($dfrom, $dto, $i_area, $i_customer, $i_product),
        );
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('G' . $h, $this->lang->line('Kelompok Harga'))
            ->setCellValue('H' . $h, $this->lang->line('HARGA JUAL'))
            ->setCellValue('I' . $h, $this->lang->line('Qty'))
            ->setCellValue('J' . $h, $this->lang->line('Jumlah'))
            ->setCellValue('K' . $h, $this->lang->line('Disk'))
            ->setCellValue('L' . $h, $this->lang->line('Total'));

        $spreadsheet->getActiveSheet()->duplicateStyle($sharedStyle3, 'A1:L5');
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('G' . $i, $row->e_price_groupname)
                    ->setCellValue('H' . $i, $row->v_unit_price)
                    ->setCellValue('I' . $i, $row->n_deliver)
                    ->setCellValue('J' . $i, $row->tot)
                    ->setCellValue('K' . $i, $row->dis)
