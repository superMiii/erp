...
class Spmb extends CI_Controller
{
	public $i_menu = '40101';
	...
	public function delete()
	{
		...
		$i_sr_id = $this->db->get_where('tm_sr', ['i_company' => $this->i_company, 'i_sr'  => $id])->row()->i_sr_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Batal ' . $this->title . ' Id : ' . $id . ' : ' . $i_sr_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
	...
	public function update_print()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Print ' . $this->title . ' Id : ' . $id . ' : ' . $i_sr_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
