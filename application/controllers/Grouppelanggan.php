...
class Grouppelanggan extends CI_Controller
{
	public $id_menu = '20401';
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $kode, $nama);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_customer_groupid = $this->db->get_where('tr_customer_group', ['i_company' => $this->i_company, 'i_customer_group'  => $id])->row()->i_customer_groupid;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_customer_groupid . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
