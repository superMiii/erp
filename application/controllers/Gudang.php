...
class Gudang extends CI_Controller
{
	...
	public function save()
	{
		...
		$this->form_validation->set_rules('i_store_id', 'i_store_id', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('e_store_name', 'e_store_name', 'trim|required|min_length[0]');
		$i_store_id = strtoupper($this->input->post('i_store_id', TRUE));
		$e_store_name = ucwords(strtolower($this->input->post('e_store_name', TRUE)));
		if ($this->form_validation->run() == false) {
			...
			$cek = $this->mymodel->cek($i_store_id);
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$pusat = 'f';
				if ($this->input->post('chk-pusat', TRUE) != null) {
					$pusat = 't';
				}

				$this->mymodel->save($i_store_id, $e_store_name, $pusat);
				if ($this->db->trans_status() === FALSE) {
					...
					$this->logger->write('Simpan ' . $this->title . ' : ' . $e_store_name . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function update()
	{
		...
		$this->form_validation->set_rules('i_store_id', 'i_store_id', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('i_store_id_old', 'i_store_id_old', 'trim|required|min_length[0]');
		$this->form_validation->set_rules('e_store_name', 'e_store_name', 'trim|required|min_length[0]');
		$i_company_old = $this->input->post('i_company_old', TRUE);
		$i_store = $this->input->post('i_store', TRUE);
		$i_store_id = strtoupper($this->input->post('i_store_id', TRUE));
		$e_store_name = ucwords(strtolower($this->input->post('e_store_name', TRUE)));
		$i_store_id_old = strtoupper($this->input->post('i_store_id_old', TRUE));
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
				'ada'	 => false,
			...
			$cek = $this->mymodel->cek_edit($i_company_old, $i_store_id, $i_store_id_old);
			/** Jika Sudah Ada Jangan Disimpan */
			if ($cek->num_rows() > 0) {
				$data = array(
					'sukses' => false,
				...
				$this->mymodel->update($i_store, $i_store_id, $e_store_name, $pusat);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
					...
					$this->db->trans_commit();
					$this->logger->write('Edit ' . $this->title . ' : ' . $e_store_name . ' : ' . $this->session->e_company_name);
					$data = array(
						'sukses' => true,
						'ada'	 => false,
					);
	...
	public function changestatus()
	{
		...
		$i_store_id = $this->db->get_where('tr_store', ['i_company' => $this->i_company, 'i_store'  => $id])->row()->i_store_id;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_store_id . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
