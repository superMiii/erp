...
class Groupbayar extends CI_Controller
{
	public $id_menu = '20402';
	...
	public function save()
	{
		...
		//$this->form_validation->set_rules('v_flafon', 'v_flafon', 'trim|required|min_length[0]');
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$v_flafon  = str_replace(',', '', str_replace(' ', '', $this->input->post('v_flafon', TRUE)));
				$this->mymodel->save($kode, $nama,$v_flafon);
	...
	public function update()
	{
		...
		if ($this->form_validation->run() == false) {
			...
			if ($cek->num_rows() > 0) {
				...
				$this->db->trans_begin();
				$this->mymodel->update($id, $kode, $nama,$v_flafon);
				if ($this->db->trans_status() === FALSE) {
					$this->db->trans_rollback();
					$data = array(
						'sukses' => false,
	...
	public function changestatus()
	{
		...
		$id = $this->input->post('id', TRUE);
		$i_price_groupid = $this->db->get_where('tr_price_group', ['i_company' => $this->i_company, 'i_price_group'  => $id])->row()->i_price_groupid;
		if ($this->form_validation->run() == false) {
			$data = array(
				'sukses' => false,
			);
			...
			if ($this->db->trans_status() === FALSE) {
				...
				$this->db->trans_commit();
				$this->logger->write('Update Status ' . $this->title . ' Id : ' . $id . ' : ' . $i_price_groupid . ' : ' . $this->session->e_company_name);
				$data = array(
					'sukses' => true,
				);
			}
