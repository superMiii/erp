...
class Infomutasihutang extends CI_Controller
{
    public $id_menu = '90410';
    ...
    public function view()
    {
        ...
        $i_supplier     = decrypt_url($this->uri->segment(3));
        $i_company         = decrypt_url($this->uri->segment(4));
        $dfrom             = decrypt_url($this->uri->segment(5));
        $dto             = decrypt_url($this->uri->segment(6));
        $data = array(
            'detail'            => $this->mymodel->get_data_detail($i_supplier, $i_company, $dfrom, $dto),
            'dfrom'                => $dfrom,
            'dto'                => $dto,
            'e_supplier_name'   => $this->logger->get_supplier($i_supplier),
        );
    ...
    public function export()
    {
        ...
        $spreadsheet->setActiveSheetIndex(0)
            ...
            ->setCellValue('B' . $h, $this->lang->line('Pemasok'))
            ->setCellValue('C' . $h, $this->lang->line('Saldo Awal'))
            ->setCellValue('D' . $h, $this->lang->line('DPP'))
            ->setCellValue('E' . $h, $this->lang->line('PPN'))
            ->setCellValue('F' . $h, $this->lang->line('Debet'))
            ->setCellValue('G' . $h, $this->lang->line('Kredit'))
            ->setCellValue('H' . $h, $this->lang->line('Saldo Akhir'));

        $spreadsheet->getActiveSheet()->duplicateStyle($sharedStyle3, 'A1:H5');
        ...
        if ($query->num_rows() > 0) {
            ...
            foreach ($query->result() as $row) {
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('C' . $i, $row->v_saldo_awal)
                    ->setCellValue('D' . $i, $row->v_dpp)
                    ->setCellValue('E' . $i, $row->v_ppn)
                    ->setCellValue('F' . $i, $row->v_debet)
                    ->setCellValue('G' . $i, $row->v_credit)
