...
class Minfokbpv extends CI_Model
{
    ...
    public function serverside()
    {
        ...
        $datatables->query("SELECT DISTINCT
                ...
                from
                    tm_pv_item a
                inner join tm_pv b on (b.i_pv=a.i_pv)
                inner join tr_coa c on (c.i_coa=a.i_coa and c.f_kas_besar='t')
                inner join tr_area d on (d.i_area=b.i_area)
                INNER JOIN tm_user_area g ON (g.i_area = d.i_area  and g.i_user = '$this->i_user')
                inner join tr_pv_type r on (r.i_pv_type=b.i_pv_type and r.i_pv_type_id='KB')
                WHERE
                b.f_pv_cancel = 'f'
                AND b.i_company = '$this->i_company'
                AND b.d_pv BETWEEN '$dfrom' AND '$dto'
                $area
                $coa
                ORDER BY
                    3 ASC
                    ", FALSE);
        $datatables->hide('i_pv_item');
    ...
    public function get_area($cari)
    {
        return $this->db->query("SELECT 
            ...
            inner join tm_pv d on (d.i_area = a.i_area)
            inner join tr_coa e on (e.i_coa=d.i_coa and e.f_kas_besar='t')
            WHERE 
