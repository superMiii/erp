...
class Mspbpromo extends CI_Model
{
    ...
    public function serverside()
    {
        ...
        $datatables->query("SELECT
                ...
                a.i_so_id,
	            P.i_promo_id || ' ~ ' || p.e_promo_name AS e_promo_name,
                ...
                to_char(a.d_approve1, 'DD Month YYYY') AS d_approve1,
                to_char(a.d_approve2, 'DD Month YYYY') AS d_approve2,
            ...
            INNER JOIN tm_user_area f ON
                (f.i_area = a.i_area)
            LEFT JOIN tm_do g ON
	            (g.i_so = a.i_so)
            left join tm_promo p on (p.i_promo=a.i_promo)
            WHERE
        ...
        $datatables->add('action', function ($data) {
            ...
            if ($i_periode >= get_periode()) {
                ...
                } elseif (($i_so_refference == '' || $i_so_refference == null) && ($i_do == '' || $i_do == null)  && $f_status == 'f') {
                    $data      .= "<a href='#' onclick='sweetdeletev33(\"" . $this->folder . "\",\"" . $id . "\",\"" . encrypt_url($dfrom)  . "\",\"" . encrypt_url($dto)  . '/' . encrypt_url($i_area) . "\");' title='Delete Data'><i class='fa fa-trash fa-lg red darken-4'></i></a>";
                }
            }
            return $data;
    ...
    public function get_promo($cari, $tanggal)
    {
        return $this->db->query("SELECT 
            ...
            WHERE 
                ...
                OR i_promo_id ILIKE '%$cari%')
                AND i_company = '$this->i_company' 
                AND f_promo_cancel = 'f'
    ...
    public function get_promo_detail($ipromo)
    {
        return $this->db->query("SELECT a.*, b.f_plus_discount
            ...
            WHERE 
                a.i_promo_type = b.i_promo_type
                AND a.i_promo = '$ipromo'
        ", FALSE);
    }

    /** Get Area */
    // public function get_area2($cari)
    // {
    //     return $this->db->query("SELECT 
    //             DISTINCT
    //             a.i_area, 
        ...
        if ($f_all_area == 't') {
            ...
            return $this->db->query("SELECT 
                ...
                WHERE 
                    ...
                    AND f_area_active = true
                    AND b.i_user = '$this->i_user' 
                    AND a.i_area IN (SELECT i_area FROM tm_promo_area WHERE i_promo = '$i_promo')
                ORDER BY 3 ASC
                ", FALSE);
        }
    }
        ...
        if ($f_all_customer == 't') {
            ...
            return $this->db->query("SELECT 
                ...
                WHERE 
                    ...
                    AND f_customer_active = 'true' 
                    AND i_area = '$i_area'
                    AND i_customer IN (SELECT i_customer FROM tm_promo_customer WHERE i_promo = '$i_promo')
                ORDER BY 3 ASC
            ", FALSE);
        }
    }
    ...
    /** Get Detail Customer */
    public function get_customer_detail($icustomer, $ipromo)
    {
        return $this->db->query("SELECT
                a.i_customer,
                ...
                d.i_promo_type_id,
                case
                    when d.i_promo_type_id = '01' then c.n_promo_discount1
                    when d.i_promo_type_id = '02' then 0
                    when d.i_promo_type_id = '03' then case when a.n_customer_discount1 = 0 then c.n_promo_discount1 else a.n_customer_discount1 end
                    when d.i_promo_type_id = '04' then a.n_customer_discount1
                    when d.i_promo_type_id = '05' then case when a.n_customer_discount1 = 0 then c.n_promo_discount1 else a.n_customer_discount1 end
                    when d.i_promo_type_id = '06' then c.n_promo_discount1
                end as n_customer_discount1,      
                case
                    when d.i_promo_type_id = '01' then c.n_promo_discount2
                    when d.i_promo_type_id = '02' then 0
                    when d.i_promo_type_id = '03' then case when a.n_customer_discount1 > 0 and a.n_customer_discount2 = 0  then c.n_promo_discount1 
                    										when a.n_customer_discount1 = 0 and a.n_customer_discount2 = 0  then c.n_promo_discount2 
                    										else a.n_customer_discount2 end
                    when d.i_promo_type_id = '04' then a.n_customer_discount2
                    when d.i_promo_type_id = '05' then case when a.n_customer_discount1 > 0 and a.n_customer_discount2 = 0  then c.n_promo_discount1 
                    										when a.n_customer_discount1 = 0 and a.n_customer_discount2 = 0  then c.n_promo_discount2 
                    										else a.n_customer_discount2 end 
                    when d.i_promo_type_id = '06' then c.n_promo_discount2
                end as n_customer_discount2,
                case
                    when d.i_promo_type_id = '01' then 0
                    when d.i_promo_type_id = '02' then 0
                    when d.i_promo_type_id = '03' then case when a.n_customer_discount1 > 0 and a.n_customer_discount2 > 0  then c.n_promo_discount1 
                    										when a.n_customer_discount1 > 0 and a.n_customer_discount2 = 0  then c.n_promo_discount2 
                    										else a.n_customer_discount1 end        
                    when d.i_promo_type_id = '04' then a.n_customer_discount3
                    when d.i_promo_type_id = '05' then case when a.n_customer_discount1 > 0 and a.n_customer_discount2 > 0  then c.n_promo_discount1 
                    										when a.n_customer_discount1 > 0 and a.n_customer_discount2 = 0  then c.n_promo_discount2 
                    										else a.n_customer_discount1 end 
                    when d.i_promo_type_id = '06' then 0
                end as n_customer_discount3,
                case
                    when d.i_promo_type_id = '01' then 0
                    when d.i_promo_type_id = '02' then 0
                    when d.i_promo_type_id = '03' then case when a.n_customer_discount1 > 0 and a.n_customer_discount2 > 0  then c.n_promo_discount2 
                    										when a.n_customer_discount1 > 0 and a.n_customer_discount2 = 0  then a.n_customer_discount3
                    										else 0 end 
                    when d.i_promo_type_id = '04' then 0
                    when d.i_promo_type_id = '05' then case when a.n_customer_discount1 > 0 and a.n_customer_discount2 > 0  then c.n_promo_discount2 
                    										when a.n_customer_discount1 > 0 and a.n_customer_discount2 = 0  then a.n_customer_discount3
                    										else 0 end
                    when d.i_promo_type_id = '06' then 0
                end as n_customer_discount4
            FROM tr_customer a
            inner join tm_promo c on(c.i_promo= '$ipromo')
            INNER JOIN tr_price_group b ON (c.i_price_group = b.i_price_group)
            inner join tr_promo_type d on(c.i_company =d.i_company and c.i_promo_type=d.i_promo_type)
    ...
    /** Get Product */
    public function get_product($cari, $i_product_group, $i_price_group, $i_promo, $f_all_product)
    {
        if ($f_all_product == 't') {
            return $this->db->query("SELECT
                    a.i_product,
            ...
            return $this->db->query("SELECT
                    a.i_product,
                case
                    when s.i_product_statusid = 'STP1' then ab.i_product_id || ' (*STP)'
                    when s.i_product_statusid = 'STP2' then ab.i_product_id || ' (#STP)'
                    else ab.i_product_id
                end as i_product_id,
                    ab.e_product_name,
                    b.e_product_motifname,
                    c.e_product_colorname
                FROM
                    tm_promo_item a
                INNER JOIN tr_product ab ON 
                    (ab.i_product = a.i_product)
                INNER JOIN tr_product_motif b ON
                    (a.i_product_motif = b.i_product_motif)
                INNER JOIN tr_product_color c ON
                    (ab.i_product_color = c.i_product_color)
            inner join tr_product_status s on
                (s.i_product_status = ab.i_product_status)
                WHERE
                    (ab.e_product_name ILIKE '%$cari%'
                        OR ab.i_product_id ILIKE '%$cari%')
                    AND ab.i_company = '$this->i_company'
                    AND a.i_promo = '$i_promo'
                    AND ab.i_product_group = '$i_product_group'
                    -- AND ab.f_product_active = TRUE
                ORDER BY
                    e_product_name ASC
            ", FALSE);
        }
    ...
    public function get_product_price($i_price_group, $i_product_group, $i_product, $i_promo, $f_all_product)
    {
        if ($f_all_product == 't') {
            return $this->db->query("SELECT
                d.v_price,
            ...
            foreach ($this->input->post('i_product') as $i_product) {
                if ($i_product != '') {
                    $item = array(
                        ...
                        'n_so_discount1'   => str_replace(",", "", $this->input->post('n_disc1')[$i]),
                        'n_so_discount2'   => str_replace(",", "", $this->input->post('n_disc2')[$i]),
                        'n_so_discount3'   => str_replace(",", "", $this->input->post('n_disc3')[$i]),
                        'n_so_discount4'   => str_replace(",", "", $this->input->post('n_disc4')[$i]),
                        'v_so_discount1'   => str_replace(",", "", $this->input->post('v_disc1')[$i]),
                        'v_so_discount2'   => str_replace(",", "", $this->input->post('v_disc2')[$i]),
                        'v_so_discount3'   => str_replace(",", "", $this->input->post('v_disc3')[$i]),
                        'v_so_discount4'   => str_replace(",", "", $this->input->post('v_disc4')[$i]),
                    );
