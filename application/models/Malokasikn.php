...
class Malokasikn extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT max(i_alokasi)+1 AS id FROM tm_alokasikn", TRUE);
        if ($query->num_rows() > 0) {
            $id = $query->row()->id;
            if ($id == null) {
                $id = 1;
        ...
        $this->db->insert('tm_alokasikn', $table);
        if (is_array($this->input->post('i_nota')) || is_object($this->input->post('i_nota'))) {
            ...
            foreach ($this->input->post('i_nota') as $i_nota) {
                if ($i_nota != '' || $i_nota != null) {
                    ...
                    $this->db->insert('tm_alokasikn_item', $item);
                    $i++;
    ...
    {
        $query = $this->db->query("SELECT i_nota, i_kn, a.v_jumlah FROM tm_alokasikn_item a, tm_alokasikn b WHERE a.i_alokasi = b.i_alokasi AND b.i_alokasi = '$id' ", FALSE);
        if ($query->num_rows() > 0) {
    ...
    public function cancel($id,$alasan,$i_alokasi_id)
    {
        ...
        $this->db->where('i_alokasi', $id);
        $this->db->update('tm_alokasikn', $table);
        $this->update_item($id);

        $this->db->query("INSERT INTO tx_del (i_company, i_user, e_user_name, e_dokumen, e_alasan)
        VALUES ('$this->i_company', '$this->i_user', '$this->e_user_name', 'No ALOKASI KN NONRETUR : $i_alokasi_id', '$alasan')");
    }
}
