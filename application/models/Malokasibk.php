...
class Malokasibk extends CI_Model
{
    ...
    public function serversidex()
    {
        ...
        $datatables->query("SELECT
            ...
            INNER JOIN tr_coa g ON 
                (g.i_coa = a.i_coa AND g.f_alokasi_bank_keluar= 't')
            INNER JOIN tr_pv_type x ON (x.i_pv_type = b.i_pv_type AND x.f_kas_bank = 't')
            ...
            WHERE
                ...
                AND a.v_pv_saldo > 0
                AND a.i_pv_refference_type NOTNULL
            ORDER BY
                1 DESC
        ", FALSE);
    ...
    public function get_supplier0($cari)
    {
        return $this->db->query("SELECT distinct
            ...
            FROM 
                tr_supplier xx
                inner join tm_alokasi_bk yy on (yy.i_supplier=xx.i_supplier)
            WHERE 
    ...
    {
        $query = $this->db->query("SELECT max(i_alokasi)+1 AS id FROM tm_alokasi_bk", TRUE);
        if ($query->num_rows() > 0) {
            $id = $query->row()->id;
            if ($id == null) {
                $id = 1;
        ...
        $this->db->insert('tm_alokasi_bk', $table);
        if (is_array($this->input->post('i_nota')) || is_object($this->input->post('i_nota'))) {
            ...
            foreach ($this->input->post('i_nota') as $i_nota) {
                ...
                $this->db->insert('tm_alokasi_bk_item', $item);
                $i++;
    ...
    public function update()
    {
        ...
        $this->db->update('tm_alokasi_bk', $table);
        if (is_array($this->input->post('i_nota')) || is_object($this->input->post('i_nota'))) {
            $this->update_item($id);
            $this->db->where('i_alokasi', $id);
            $this->db->delete('tm_alokasi_bk_item');
            $i = 0;
            foreach ($this->input->post('i_nota') as $i_nota) {
                $v_jumlah = str_replace(",", "", $this->input->post('vjumlah')[$i]);
                $item = array(
    ...
    {
        $query = $this->db->query("SELECT i_nota, i_pv_item, v_jumlah FROM tm_alokasi_bk_item WHERE i_alokasi = '$id' ", FALSE);
        if ($query->num_rows() > 0) {
    ...
    public function cancel($id,$alasan,$i_alokasi_id)
    {
        ...
        VALUES ('$this->i_company', '$this->i_user', '$this->e_user_name', 'No ALOKASI BK : $i_alokasi_id', '$alasan')");
    }
}
