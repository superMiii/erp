...
class Minfospbcus3 extends CI_Model
{
    ...
    public function serverside()
    {
        ...
        from (
		        ...
		        inner join tr_customer k on (k.i_customer=mdo.i_customer and k.f_pareto='t')
...
b.i_customer ,
a.i_product ,
to_char(b.d_do , 'MM') as bulan, sum(coalesce(a.n_deliver,0)) as n_deliver  
from tm_do_item a
...
$$,
$$
 select to_char(DATE '1001-01-01' + (interval '1' month * generate_series(0, 11)), 'MM') as bulan
$$)
as ( i_customer numeric,i_area numeric, i_product numeric, jan text, feb text, mar text, apr text, 
mei text, jun text, jul text, aug text, sep text, okt text, nov text, de text );
 */

/* 
with raya as (
    select distinct  b.i_customer ,a.i_product ,y.bulan
    from
        tm_do_item a
    inner join tm_do b on (b.i_do  = a.i_do)
    cross join ( select to_char(DATE '1001-01-01' + (interval '1' month * generate_series(0, 11)), 'MM') as bulan) y
    ...
    from ( select 
            ...
            group by 1,2,3
            order by 1,2 ) as x
    right join raya y on (x.i_customer = y.i_customer and x.i_product = y.i_product and x.bulan = y.bulan)
    group by 1,2
    order by 1,2
     */