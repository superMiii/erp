...
class Msj extends CI_Model
{
    ...
    public function serverside()
    {
        ...
        $datatables->query("SELECT DISTINCT
            ...
            inner join tm_user us on (us.i_user=g.i_user)
            LEFT JOIN (SELECT DISTINCT i_do, i_sl_id, a.d_sl  FROM tm_sl a, tm_sl_item b WHERE a.i_sl = b.i_sl AND a.f_sl_batal = 'f') h ON (h.i_do = a.i_do and a.f_do_cancel = 'f') 
            left join tm_nota_item ww on (ww.i_do=a.i_do)
            WHERE
                a.i_company = '$this->i_company'
                AND g.i_user = '$this->i_user'
                And case when '$f_pusat' = 'f' then c.f_so_stockdaerah='t' else us.f_status = 't' end
                AND a.d_do BETWEEN '$dfrom' AND '$dto'
                $area
                And h.i_sl_id isnull
                and f_do_cancel = 'f'
            ORDER BY
                1 ASC
        ", FALSE);
        ...
        $datatables->add('action', function ($data) {
            ...
            if ($i_periode >= get_periode3()) {
                if (check_role($this->id_menu, 3) && $f_status == 'f' && ($i_nota == '' || $i_nota == null)) {
                ...
                if (check_role($this->id_menu, 4) && $f_status == 'f' && ($i_sl_id == '' || $i_sl_id == null)) {
                    $data      .= "<a href='#' onclick='sweetdeletev33raya2(\"" . $this->folder . "\",\"" . $id . "\",\"" . encrypt_url($dfrom)  . "\",\"" . encrypt_url($dto)  . '/' . encrypt_url($i_area) . "\");' title='Delete Data'><i class='fa fa-trash fa-lg red darken-4'></i></a>";
                }
            }
            return $data;
        });
    ...
    public function save_turunan()
    {
        ...
        if ($this->input->post('jml') > 0) {
            ...
            $this->db->query("
				...
				select i_company, '$id_so' as i_so, '$number' as i_so_id, i_area, i_customer, i_salesman, i_price_group, null as i_store, null as i_store_location,case when f_so_stockdaerah = 't' then 2 else 1 end as i_status_so, i_product_group, '$d_so' as d_so, now() as d_so_entry, 
            ...
            foreach ($this->input->post('i_product') as $i_product) {
                ...
                if ($n_turunan > 0) {
                    ...
                    $this->db->query("UPDATE tm_so_item SET n_order = '$n_deliver' WHERE i_so = '$i_so' AND i_product = '$i_product'");
    ...
    /** Cancel Data */
    public function cancel($id,$alasan,$i_do_id)
    {
        $table = array(
            'f_do_cancel' => 't',
        );
        ...
        VALUES ('$this->i_company', '$this->i_user', '$this->e_user_name', 'No SJ : $i_do_id', '$alasan')");
