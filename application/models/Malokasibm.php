...
class Malokasibm extends CI_Model
{
    ...
    {
        $query = $this->db->query("SELECT max(i_alokasi)+1 AS id FROM tm_alokasi", TRUE);
        if ($query->num_rows() > 0) {
            $id = $query->row()->id;
            if ($id == null) {
                $id = 1;
        ...
        $this->db->insert('tm_alokasi', $table);
    ...
    {
        $query = $this->db->query("SELECT i_nota, i_rv_item, v_jumlah FROM tm_alokasi_item WHERE i_alokasi = '$id' ", FALSE);
        if ($query->num_rows() > 0) {
    ...
    public function cancel($id,$alasan,$i_alokasi_id)
    {
        ...
        VALUES ('$this->i_company', '$this->i_user', '$this->e_user_name', 'No ALOKASI BM : $i_alokasi_id', '$alasan')");
    }
}
