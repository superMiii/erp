...
class Minfomutasistok extends CI_Model
{
    ...
    public function serverside()
    {
        ...
        $datatables->query("SELECT
                ...
                n_selisi,
                n_git,
	            coalesce (ngit, 0) as ngit,
                '$dfrom' AS dfrom,
                '$dto' AS dto
                FROM f_mutasi_stock4($this->i_company,'$i_periode','$datefrom','$dateto','$datejangkafrom','$datejangkato') as tt
                left join (select i_company,i_product,sum(n_git) as ngit 
                from (select * from	tz_git where d_gs BETWEEN '$datefrom' AND '$dateto' and i_company=$this->i_company) as tx group by 1,2) as tz
                on (tz.i_company=tt.i_company and tz.i_product=tt.i_product)
                order by 5
                ", FALSE);
